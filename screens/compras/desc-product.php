            ...
            while ($row_prod = mysqli_fetch_assoc($result_page)) { ?>
                <div class='group shadow-md rounded-lg'>
                    <a href='desc-product.php?id=<?php echo $row_prod['id_prod'] ?>'>
                        ...
                        <div class='p-3'>
                            <h3 class='text-base text-gray-700 truncate'><?php echo $row_prod['nome_prod'] ?></h3>
                            <p class='flex items-center justify-between mt-1 text-lg font-medium text-gray-900'>R$ <?php echo number_format($row_prod['preco_custo_prod'], 2, ",", ".") ?></p>
                            <?php
                            if ($row_prod['qtd_estoque'] > 0) { ?>
                                <a href='../compras/cart/template_store/cart.php'>
